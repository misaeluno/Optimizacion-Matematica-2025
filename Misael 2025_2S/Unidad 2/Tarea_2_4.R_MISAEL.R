...
x_vals <- seq(-2, 2, length = 50)
y_vals <- seq(-2, 2, length = 50)

# Crear la función que outer() puede usar
f_3d <- function(x, y) {
  return(0.5*x^2 + 2.5*y^2 - 2*x*y - x^3)
}

# Crear la malla de valores Z
z <- outer(x_vals, y_vals, f_3d)

# Graficar la superficie 3D
persp(x_vals, y_vals, z,
      col = "skyblue",
      theta = 30, phi = 20,     # Ángulos de vista
      expand = 1,
      shade = 1.2,
      xlab = "X", ylab = "Y", zlab = "f(x, y)",
      main = "Superficie 3D de la Función Objetivo")
