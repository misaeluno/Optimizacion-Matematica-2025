...
for i, (x_opt, tray) in enumerate(resultados, 1):
    print(f"RESULTADO {i}")
    print(f"Punto óptimo encontrado: x = {x_opt}")
    print(f"Valor de la función: f(x) = {f(x_opt):.10f}")
...
for i, (x_opt, tray) in enumerate(resultados):
    ...
    z_tray = [f(punto) for punto in tray]
...
for i, (x_opt, tray) in enumerate(resultados):
    ...
    ax2.plot(tray_array[:, 0], tray_array[:, 1], 
         '.-', color=colores[i], linewidth=2, markersize=6,
         label=f'Trayectoria {i+1}')
    
    ax2.scatter(puntos_iniciales[i][0], puntos_iniciales[i][1], 
                color=colores[i], s=100, marker='o', zorder=5)
