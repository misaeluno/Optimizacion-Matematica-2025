...
gradiente <- function(beta, t_data, y_data) {
  ...
  for(i in 1:n) {
    ...
    dy_dA <- exp_term * cos_term                  #deribada de Y segun A
    dy_dlanda <- -A * t * exp_term * cos_term     #deribada de Y segun landa
    dy_dw <- -A * exp_term * sin_term * t         #deribada de Y segun W
    dy_dteta <- -A * exp_term * sin_term          #deribada de Y segun Teta
    #-------------------------------------------------------------------
    # Gradiente (regla de la cadena [NOOO de nuevo jajajjja XD]) 
    grad[1] <- grad[1] - 2 * r * dy_dA
    grad[2] <- grad[2] - 2 * r * dy_dlanda
    grad[3] <- grad[3] - 2 * r * dy_dw
    grad[4] <- grad[4] - 2 * r * dy_dteta
  }    
...
hessiana <- function(beta, t_data, y_data) {
  ...
  n <- length(t_data)
  #---------------------------------------------------------------------
  #Matris para la hessiana
  H <- matrix(0, nrow = 4, ncol = 4)
  
  for(i in 1:n) {
    # rescribiendo t segun la base de datos
    t <- t_data[i]
    #-------------------------------------------------------------------
    # Predicción
    exp_term <- exp(-landa * t)
    ...
    dy_dteta <- -A * exp_term * sin_term          #deribada de Y segun Teta
    #-------------------------------------------------------------------
    # Derivadas de segundo orden (aproximación de Gauss-Newton)
    # H = 2 * J**T * J (ignorando términos de segundo orden con residuos)
    J <- c(dy_dA, dy_dlanda, dy_dw, dy_dteta)
    H <- H + 2 * outer(J, J)
  }
...
newton <- function(beta_0, t_data, y_data, tol = 1e-6, max_iter = 100) {
  #BETA tiene los valores de A, landa, W y TETA
  beta <- beta_0
