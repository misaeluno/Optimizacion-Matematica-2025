...
# Asignar el punto según la elección (CORREGIDO)
if (opcion == "1") {
  x_actual <- punto_1
  nombre_punto <- "( 1.5,  1.0)"
} 
else if (opcion == "2") {
  x_actual <- punto_2
  nombre_punto <- "(-1.5, -1.0)"
  ...
  x_actual <- punto_3
  nombre_punto <- "( 0.1, -0.2)"
} 
else {
  cat("Opción inválida. Usando punto por defecto: (0.1, -0.2)\n")
  x_actual <- punto_3
  nombre_punto <- "( 0.1, -0.2)"
}

cat("\nPunto inicial seleccionado:", nombre_punto, "\n")
cat(rep("=", 60), "\n\n", sep="")
#-------------------------------------------------------------------------------
